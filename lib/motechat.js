...
var regAgainAll = function(){
    ...
    if ( ix >= 0 ){
        ...
        //dcData = {"AppKey":data.AppKey,"EiToken":data.EiToken,"SToken":SToken,"EiUMMA":data.EiUMMA,"EiUPort":data.EiUPort,"WIP":data.WIP,"LIP":data.LIP};
            ...
            if ( dbg >= 1 ) console.log('motechat:regAgainAll: data=%s', JSON.stringify(dcData));
            tryreg( dcData, function(result){
                if ( dbg >= 1 ) console.log('motechat:regAgainAll result=%s', JSON.stringify(result));
                InStateHandler('re-reg ' + result.ErrMsg, data.DDN);
            });
        }
 ...
 * @param {String} xmsg.To        device ddn of destination (legacy, backward comaptible)
...
//      To: device ddn of destination (legacy, backward comaptible)
...
exports.Send = function(xmsg, cb){
    var stoken, topic, to, dest, addr, data, timeout, waitreply, timeadj;
    var ix;
    try {
        if ( mcState != 'open' ){
        ...
        if ( topic ){
            if ( typeof topic == 'string' ){
                ix = topic.indexOf('://');
                if ( ix > 0 ) dest = topic.substr(ix+3);
                else dest = topic;
                ix = dest.indexOf('/');
                if ( ix > 0 ) dest = dest.substr(0, ix);
                addr = {"Target":dest,"Topic":topic};
            }
            else {
                addr = topic;
            }
        ...
        if ( info != null ){
            ...
            var xdata = {"stoken":stoken,"in":{"fm":fm,"msgtype":"","t1":timeout,"t2":waitreply},"to":addr,"data":data};
                ...
                function(result){
                    ...
                    if ( typeof cb == 'function' ){
                        if ( result.body ) {
                            ...
                            if ( result.ErrCode ) cb([{"IN":{"From":fm,"To":addr,"State":result.body},"Reply":""}]);
                            else if ( result.IN ) cb(result);
                            else cb([{"IN":{"To":addr,"State":{"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg,"result":result}},Reply:""}]);
                        }
                    }
                    ...
                    if ( iocmma != '' ){
                        if ( result.body ){
                            ...
                            if ( body.ErrCode ){
                                edata.From = fm;
                                edata.To = addr;
                                edata.msg = xdata.data;
                                edata.result = body.ErrMsg;
                                console.log('motechat:Send edata=%s', JSON.stringify(edata));
                                ...
                                for ( var i = 0; i < body.length; i++ ) {
                                    ...
                                    console.log('motechat:Call edata=%s', JSON.stringify(edata));
                        ...
                        console.log('motechat:Send unexpected result=%s', JSON.stringify(result));
                    }
                }
            );
        }
 ...
 * @example 
    ...
    var to = 'myFunc'; (legacy, backward comaptible)
    var func = 'echo';
 ...
 * @param {String} xrpc.To          device ddn of destination (legacy, backward comaptible)
...
//      To: device ddn or name of destination (legacy, backward comaptible)
...
exports.Call = function(xrpc, cb){
    var stoken, target, to, topic, dest, addr, func, data, timeout, waitreply;
    var ix;
    addr = {"Target":"","Topic":""};
    try {
        if ( mcState != 'open' ){
        ...
        //if ( dbg >= 1 ) {
        //    if ( typeof data == 'object')
        //        console.log('motechat:Call stoken=%s,addr=%s,func=%s,data=%s', stoken, JSON.stringify(addr), func, JSON.stringify(data));
        //    else
        //        console.log('motechat:Call stoken=%s,addr=%s,func=%s,data=%s', stoken, JSON.stringify(addr), func, data);
        //}
        if ( !stoken || !addr || !func ){
            if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg}); 
            return;
        ...
        var info = GetEdgeInfo('stoken', stoken);
        if ( info != null ){
            if ( addr.Target == 'UC' || addr.Target == 'uc' ){
                if ( ucState == 'start' ){
                    try {
                ...
                if ( uid == '' && info.EiOwner ) uid = info.EiOwner;
                var fm = {"DDN":ddn,"Name":dname,"Type":dtype,"Uid":uid};
                var dcData = {"stoken":stoken,"in":{"fm":fm,"t1":timeout,"t2":waitreply},"to":addr,"func":func,"data":data};
                console.log('motechat:Call data=%s', JSON.stringify(dcData));
                ins.CallXrpc( dcenter, 'call', dcData, timeout, waitreply, function(result){
                    ...
                    if ( typeof cb == 'function' ) {
                        //if ( result ) cb( result );
                        if ( result.ErrCode ) cb([{"IN":{"From":fm,"To":addr,"State":result},Reply:""}]);
                        else cb(result);
                    }
                    if ( iocmma != '' ){
                        if ( result.ErrCode ){
...
var InStateHandler = function(state, ddn){
    ...
    if ( state == 'opened2' ){
        ...
        MBusRestart( function(result){
            if ( dbg >= 0 ) console.log('motechat:InStateHandler:MBusRestart result=%s',JSON.stringify(result));
        });
    }
}
...
var MBusRestart = function(cb){
    ...
    setTimeout(function(){
        var tm = 2000 + Math.floor((Math.random() * 10) + 1) * 100;
        CallDcenterReset(function(reply){
            console.log('motechat:MBusRestart reply=%s', JSON.stringify(reply));
            if ( reply.ErrCode == mcerr.MC_OKCODE ){
                // DC response OK
                if ( regflag == true ){
                    var data = {"SToken":reginfo.SToken,"EiToken":reginfo.EiToken,"WIP":mcwanip,"Web":webtype};
...
/*
var MoteChatGetHandler2 = function(ch, head, body, cb){
    var from, to, ddn, data;
    var msgtype = '';
    if ( dbg >= 1 ) console.log('motechat:MoteChatGetHandler body=%s',JSON.stringify(body));
    if ( body.in ){
    ...
    if ( dcenter != '' ){
        ins.CallXrpc( dcenter, 'poll', 'poll', null, null, function(reply){
            ...
            if ( reply.ErrCode == mcerr.MC_OKCODE ){
                ...
                ins.iocEvent('', mbusmma, 'error', 'in', {"Device":mbusmma,"action":"keepalive","result": "dc: " + reply.ErrMsg});
            }
        });
    }    
}

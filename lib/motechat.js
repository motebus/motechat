...
var startReg = function(delay, opencb){
    var tm;
...
exports.Call = function(xrpc, cb){
    ...
    try {
        ...
        if ( info != null ){
            if ( addr.to.Target == 'UC' || addr.to.Target == 'uc' ){
                if ( ucState == 'start' ){
                    try {
            ...
            if (cmd){
                ...
                switch(cmd){
                    case 'ping':
                        ...
                        if ( indata.in.fm.DDN == indata.to.DDN || indata.to.DDN == 'local' ){
                            result = {"ErrCode":mcerr.MC_OKCODE,"ErrMsg":mcerr.MC_OKMSG};
                            if ( typeof cb == 'function' ) cb([{"IN":{"From":indata.in.fm,"To":indata.to,"State":result},"Reply":{"response":"ping","Trace":stamp}}]);
                            return true;
                        }
                        else {
                    ...
                    case 'trace':
                        ...
                        if ( indata.in.fm.DDN == indata.to.DDN || indata.to.DDN == 'local' ){
                            result = {"ErrCode":mcerr.MC_OKCODE,"ErrMsg":mcerr.MC_OKMSG};
                            if ( typeof cb == 'function' ) cb([{"IN":{"From":indata.in.fm,"To":indata.to,"State":result},"Reply":{"response":"trace","Trace":stamp}}]);
                            return true;
                        }
                        else {
                            indata.data = {"cmd":cmd,"option":option,"trace":stamp};

...
exports.Open = function(conf, reg, callback){
    if ( openFlag == false ){
        ...
        if ( isOpened == false ){
            ...
            // has opened
            console.log('motechat:Open mchatReset');
            if ( typeof cb == 'function' ){
                try {
                    mchatReset(reginfo, cb);
                    openFlag = false;
                }
                catch(err){
                    cb({"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":err.message});
                ...
                return new Promise(function(resolve) {
                    try {
                        mchatReset(reginfo, function(result){
                            resolve(result);
                            openFlag = false;
                        });
                    }
    ...
    if ( mcState == '' ){
        ...
        ins.Open( conf, function(result){
            ...
            if ( result.ErrCode == mcerr.MC_OKCODE ) {
                ...
                ins.PublishXrpc( appname, MotechatService, cb );
...
var mchatReset = function(reginfo, cb){
    var opencb = cb;
    connDC(null, reginfo, function(result){
        if ( typeof opencb == 'function') opencb(result);
        openFlag = false;
        isOpened = true;
    });
                                ...
                                setInfo( reginfo, function(ret){
                                    if ( ret.ErrCode == mcerr.MC_OKCODE ){
                                        if ( typeof opencb == 'function' ) {
                                            let ei = reginfo.EdgeInfo;
                                            let dinfo = {"EiName":ei.EiName,"EiType":ei.EiType,"EiTag":ei.EiType,"EiLoc":ei.EiLoc};
                                            let regret = {"ErrCode":mcerr.MC_OKCODE,"ErrMsg":mcerr.MC_OKMSG,"result":dinfo};
                                            opencb(regret);
                                        }
                                    }
                                    else {
...
function regCenter()  {
    ...
    var _watch = function(){
        if ( dcState == 'open' ){
            ...
            let tm = _tm + Math.floor((Math.random() * 10) + 1);
...
exports.Send = function(xmsg, cb){
    try {
        ...
        let selfinfo = null;
        if ( SToken ){
            selfinfo = GetEdgeInfo('stoken', SToken);
        }
        let Addr = null;
        if ( selfinfo ) {
            if ( DDN ){
                                ...
                                for ( var i = 0; i < cdata.length; i++ ) {
                                    ...
                                    if ( msg.IN ){
                                        ...
                                        if ( From && To && State ){
                                            ...
                                            let edata = {"From":From,"To":To,"msg":xdata.data,"result":ret};
            ...
            if ( !Addr || !Data ){
                ...
                if ( Addr.to.DDN == 'UC' || Addr.to.DDN == 'uc' || Addr.to.Target == 'UC' || Addr.to.local_target_prefix == 'uc' ){
                    ...
                    if ( ucState == 'start' ){
                        try {
                            let {EiMMA} = selfinfo;
                            var ucdata = {"EiMMA":EiMMA,"SToken":SToken,"Func":Func,"Data":Data};
                            ...
                            if ( iocmma ){
                                for ( var i = 0; i < cdata.length; i++ ) {
                                    ...
                                    if ( msg.IN ){
                                        ...
                                        if ( From && To && State ){
                                            if ( State.ErrMsg ) ret = State.ErrMsg;
                                            let edata = {"From":From,"To":To,"msg":dcData.data,"result":ret};
                                            if ( dbg >= 2 ) console.log('motechat:Send edata=%s', JSON.stringify(edata));
                                            if ( ret == mcerr.MC_OKMSG )
...
var ReReg = function(stoken){
    ...
    if ( i >= 0 ) {
        ...
        regfunc.reg(regdata, 'user', function(reply){

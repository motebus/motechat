...
exports.Open = function(conf, reg, callback){
    ...
    if ( arguments.length == 1 ){
        ...
        if ( reg ) {
            reginfo = reg;
            if ( !reginfo.SToken && !reginfo.EiToken ) regflag = true;
        }
...
function regCenter()  {
    ...
    this.dcreset = function(){
        ...
        for ( let i = 0; i < edgetable.length; i++ ){
            ...
            let dcData = {"EiToken":ei.EiToken,"SToken":ei.SToken,"WIP":ei.WIP,"Web":ei.Web,"EiInfo":info};
        ...
        if ( data.SToken ){
            ...
            if ( info ){
                ins.CallXrpc( dcenter, 'nearby', data, null, null, function(reply){
                    if ( dbg >= 0 ) console.log('motechat:Nearby reply=%s', JSON.stringify(reply));
                    if ( reply.ErrCode ){

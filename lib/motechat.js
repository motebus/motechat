 ...
 * @example
    var conf = { "AppName":"", "IOC":"", "DCenter":"", "UCenter":"", "AppKey":"", "UseWeb":"" }
	conf.AppName = 'myfunc';
...
exports.Open = function(conf, reg, callback){
    ...
    if ( arguments.length == 1 ){
        ...
        if ( reginfo.SToken && reginfo.EiToken )
            regflag = true;
    }
    else {
 ...
 * @example 
    ...
    var topic = 'steven.bot';
    var to = 'myEi';
    var data = {"message":"Hello World"};
    var t1 = 6;
    var t2 = 12;
    var xmsgctl = {"SToken":stoken,"Topic":topic,"To":to,"Data":data, "SendTimeout":t1,"WaitReply":t2};
 ...
 * @param {String} xmsg.Topic     Topic of app
 * @param {String} xmsg.To        the ultranet address of destination
...
//      SToken: token of app
//      Topic: the topic data of source
//      To: the ultranet address of destination
//      Target: the target name of destination (legacy, backward comaptible)
...
exports.Send = function(xmsg, cb){
    var stoken, topic, target, to, data, timeout, waitreply;
    to = {};
    try {
        if ( mcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg}}});
            return;    
        }
        if ( dcenter == '' ){
            if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_NullDC,"ErrMsg":mcerr.MC_NullDC_Msg}}});
            return;
        }
        stoken = ( xmsg.SToken ) ? xmsg.SToken : '';
        target = ( xmsg.Target ) ? xmsg.Target : '';
        topic = ( xmsg.Topic ) ? xmsg.Topic : '';
        if ( xmsg.To ){
            if ( typeof xmsg.To == 'string' ) to = {"Target":xmsg.To,"Topic":""};
            else to = xmsg.To;
        }
        ...
        if ( timeout == null ) timeout = DefaultXmsgTimeout;
        if ( waitreply == null ) waitreply = DefaultWaitTimeout;
        if ( dbg >= 1 ) {
            if ( typeof data == 'object')
                console.log('motechat:Send to=%s,data=%s', JSON.stringify(to), JSON.stringify(data));
            else
                console.log('motechat:Send to=%s,data=%s', JSON.stringify(to), data);
        }
        if ( stoken == '' || ( to.Target && to.DDN ) || data == '' ){
            if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg}}}); 
            return;
        }
        ...
        if ( info != null ){
            ...
            var fm = {"DDN":ddn,"Name":dname,"Type":dtype,"Uid":uid,"Topic":topic};
            if ( dbg >= 1 ) console.log('motechat:Send fm DDN=%s,EiName=%s,to=%s,data=%s', ddn, dname, JSON.stringify(to), JSON.stringify(data));    
            var xdata = {"stoken":stoken,"in":{"fm":fm,"msgtype":"","t1":timeout,"t2":waitreply},"to":to,"data":data};
            ins.SendXmsg(dcenter, xdata, [], timeout, waitreply,
                function(result){
                    if ( dbg >= 1 ) console.log('motechat:Send: result=%s', JSON.stringify(result));
                    if ( typeof cb == 'function' ){
                        if ( result.body ) {
                            //cb({"IN":{"From":fm,"To":to,"State":result.body}});
                            cb( result.body );
                        }
                        else {
                            cb( result );
                    ...
                    if ( iocmma != '' ){
                        ...
                        if ( result.body ) {
                            if ( result.body.IN ){
                                if ( result.body.Reply ) edata = {"IN":result.body.IN,"result":result.body.Reply};
                                else edata = {"IN":result.body.IN};
                            }
                            else {
                                edata = {"IN":{"From":fm,"To":to,"State":result.body}};
                            }
                        }
                        else {
                            edata = {"IN":{"From":fm,"To":to,"State":result}};
                        }
            ...
            if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_InvalidSToken,"ErrMsg":mcerr.MC_InvalidSToken_Msg}}});
        }
    }
    catch(err){
        if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":"mc: " + err.message}}});
    }
}
 ...
 * @example 
    ...
    var t1 = 6;
    var t2 = 12;
    var xrpc = {"SToken":mydev.SToken, "Topic":topic ,"To":to,"Func":func,"Data":data, "SendTimeout":t1, "WaitReply":t2};
 ...
 * @param {String} xrpc.Topic     Topic of app
 * @param {String} xrpc.To        the ultranet address of destination
 * @param {String} xrpc.Target the target name of function  (legacy, backward comaptible)
...
exports.Call = function(xrpc, cb){
    var stoken, target, to, topic, func, data, timeout, waitreply;
    to = {};
    try {
        if ( mcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg}}});
        ...
        console.log('motechat:Call xrpc=%s', JSON.stringify(xrpc));
        stoken = ( xrpc.SToken ) ? xrpc.SToken : '';
        target = ( xrpc.Target ) ? xrpc.Target : '';
        topic = ( xrpc.Topic ) ? xrpc.Topic : '';
        if ( xrpc.To ){
            if ( typeof xrpc.To == 'string' ) to = {"Target":xrpc.To,"Topic":""};
            else to = xrpc.To;
        }
        else to = {"Target":target,"Topic":""};
        ...
        if ( timeout == null ) timeout = DefaultXrpcTimeout;
        if ( waitreply == null ) waitreply = DefaultWaitTimeout;
        if ( dbg >= 1 ) {
            if ( typeof data == 'object')
                console.log('motechat:Call stoken=%s,to=%s,func=%s,data=%s', stoken, JSON.stringify(to), func, JSON.stringify(data));
            else
                console.log('motechat:Call stoken=%s,to=%s,func=%s,data=%s', stoken, JSON.stringify(to), func, data);
        }
        if ( stoken == '' || ( to.Target && to.DDN ) || func == '' ){
            if ( typeof cb == 'function' ) cb({"IN":{"To":to,"State":{"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg}}}); 
            return;
        }
        var info = GetEdgeInfo('stoken', stoken);
        if ( info != null ){
            if ( to.Target == 'UC' || to.Target == 'uc' ){
                if ( ucState == 'start' ){
                    try {
                ...
                var dname = info.EiName;
                var dtype = info.EiType;
                var uid = info.Uid != null ? info.Uid : '';
                var fm = {"DDN":ddn,"Name":dname,"Type":dtype,"Uid":uid,"Topic":topic};
                var dcData = {"stoken":stoken,"in":{"fm":fm,"t1":timeout,"t2":waitreply},"to":to,"func":func,"data":data};
                ins.CallXrpc( dcenter, 'call', dcData, timeout, waitreply, function(result){
                    ...
                    if ( typeof cb == 'function' ) {
                        if ( result ) cb( result );
                        //if ( result.IN ) cb(result);
                        //else cb({"IN":{"From":fm,"To":to,"State":result}});
                    }
                    if ( iocmma != '' ){
                        var edata;
                        if ( result.IN ){
                            if ( result.Reply ) edata = {"IN":result.IN,"result":result.Reply};
                            else edata = {"IN":result.IN};
                        }
                        else {
                            edata = {"IN":{"From":fm,"To":to,"State":result}};
                        }
    ...
    if ( dcenter != '' ){
        ins.CallXrpc( dcenter, 'poll', 'poll', null, null, function(reply){
            ...
            if ( reply.ErrCode == mcerr.MC_OKCODE ){
                ...
                ins.iocEvent('', mbusmma, 'error', 'in', {"Device":mbusmma,"action":"poll","result": "dc: " + reply.ErrMsg});
            }
        });
    }    
}

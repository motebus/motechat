...
// Date: 2019/09/17
// Version: 1.6.5
// Update: solve the bug of mcstate and uc module

var exports = module.exports = {};
var ver = '1.6.5';
var update = '2019/09/17'
var appname = '';
var iocmma = '';
var webtype = '';
var dcenter = '';
...
exports.Open = function(conf, reg, callback){
    ...
    if ( typeof cb == 'function' ){
        ...
        return new Promise(function(resolve) {
            try {
                if ( conf ){
                    mchatOpen(conf, reginfo, function(result){
                        resolve(result);
                    });
                }
                else {
                    resolve({"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg});
                }
            }
            catch(err){
                cb({"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":err.message});
...
var mchatOpen = function(conf, reginfo, cb){
    ...
    if ( conf.UCenter ) {
        ucenter = conf.UCenter;
        if ( ucenter ){
            if ( ucenter.indexOf('@') < 0 ) ucenter = '';
        }
    }
    ...
    if ( mcState == '' ){
        ...
        ins.Open( conf, function(result){
            console.log('motechat:open: in Open result=%s', JSON.stringify(result));
    ...
    CallDcenterReset(function(reply){
        ...
        if ( reply.ErrCode == mcerr.MC_OKCODE ){
            ...
            if ( reginfo ){
                ...
                data = {"SToken":reginfo.SToken,"EiToken":reginfo.EiToken,"WIP":reginfo.WIP,"LIP":reginfo.LIP,"Web":webtype};
                ...
                regfunc.start(data, 'user', function(result){
                    ...
                    if ( needset && result.ErrCode == mcerr.MC_OKCODE ){
                        setInfo( reginfo, function(ret){
                            if ( ret.ErrCode == mcerr.MC_OKCODE ){
                                if ( typeof opencb == 'function' ) {
                                    let regret = result.result;
                                    let ei = reginfo.EdgeInfo;
                                    regret.EiName =ei.EiName;
                                    regret.EiType =ei.EiType;
                                    regret.EiTag =ei.EiTag;
                                    regret.EiLoc =ei.EiLoc;
                                    opencb(result);
                                }
                            }
                            else {
    ...
    if ( typeof cb == 'function' ){
        ...
        return new Promise(function(resolve, reject){
            try {
                ...
                reject({"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":err.message});
            }
        });
    }
}
    ...
    var _restart = function(id){
        if ( id >= 0 ){
            ...
            if ( regctl ){
                ...
                _reg(data, 're-reg', function(result){
                    console.log('regCenter:_restart reg result=%s', JSON.stringify(result));
                    if ( result.ErrCode == mcerr.MC_OKCODE ) {
                        if ( typeof cb == 'function' ){
...
var unreg = function(SToken, cb){
    if ( dcenter != '' ){
        let data = GetEdgeInfo('stoken',SToken);
        if ( data != null ){
            //console.log('motechat:UnReg data=%s', JSON.stringify(data));
            let {EiName,DDN} = data; 
...
exports.Send = function(xmsg, cb){
    ...
    try {
        ...
        timeout = ( typeof xmsg.SendTimeout != 'undefined' ) ? xmsg.SendTimeout : null;
        waitreply = ( typeof xmsg.WaitReply != 'undefined' ) ? xmsg.WaitReply : null;
        if ( timeout == null ) {
...
exports.Call = function(xrpc, cb){
    ...
    try {
        ...
        timeout = ( typeof xrpc.SendTimeout != 'undefined' ) ? xrpc.SendTimeout : null;
        waitreply = ( typeof xrpc.WaitReply != 'undefined' ) ? xrpc.WaitReply : null;
        if ( timeout == null ) {
        ...
        var info = GetEdgeInfo('stoken', stoken);
        if ( dbg >= 1 ) console.log('motechat:Call addr=%s,func=%s,data=%s', JSON.stringify(addr),func,JSON.stringify(data));
        if ( info != null ){
...
var ReReg = function(stoken){
    ...
    if ( i >= 0 ) {
        var regdata = {"EiToken":edgetable[i].EiToken,"SToken":edgetable[i].SToken,"WIP":edgetable[i].WIP,"LIP":edgetable[i].LIP,"Web":edgetable[i].Web};
...
var setInfo = function(data, cb){
    if ( data.SToken ){
        ...
        let ginfo = data.EdgeInfo;
        if ( stoken && ginfo ){
            let info = GetEdgeInfo('stoken', stoken);
            if ( info ){
                if ( CompareEdgeInfo(ginfo, info) == false){
                    ins.CallXrpc( dcenter, 'setinfo', data, null, null, function(reply){
                        ...
                        if ( reply.ErrCode ){
                            if ( reply.ErrCode != mcerr.MC_OKCODE ) MoteErrHandler('set', reply.ErrMsg, data.SToken);
                                ...
                                var uinfo = {"SToken":stoken,"EiName":ginfo.EiName,"EiType":ginfo.EiType,"EiTag":ginfo.EiTag,"EiLoc":ginfo.EiLoc};
                                UpdateEdgeInfo(uinfo);
                            }
                        }
                        if ( typeof cb == 'function' ) cb(reply);
...
var InStateHandler = function(state, ddn){
    if ( dbg >= 0 ) console.log('motechat:InStateHandler state=%s',state);
...
var CallDcenterReset = function(cb){
    if ( mcState != 'open' ){
        if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg});
        let tm = 3000 + Math.floor((Math.random() * 10) + 1) * 200;
            ...
            if ( type == 'in'){
                if ( cmd == 'reginfo' ){
                    ...
                    CallDcenterReset(function(result){
                        ...
                        console.log('motechat:reset DCReset result=%s', JSON.stringify(result));
...
var NewEdgeInfo = function(stoken, appkey, eitoken, eiumma, eiuport, wip, lip, web, ei){
    ...
    "DDN":ei.DDN,"EiOwner":ei.EiOwner,"EiName":ei.EiName,"EiType":ei.EiType,"EiTag":ei.EiTag,
    ...
    try {
        ...
        if ( stoken ){
            ...
            if ( i < 0 ){
                ...
                edge.State = 'conn';
                //console.log('motechat:UpdateEdgeInfo edgeinfo=%s', JSON.stringify(edge));
                return i;
            }
        }
        else {
...
var PollDC = function(){
    if ( dcenter && dcState == 'open' && edgetable.length > 0 ){
        ins.CallXrpc( dcenter, 'poll', 'poll', null, null, function(reply){
            ...
            if ( reply.ErrCode == mcerr.MC_OKCODE ){
                if ( reply.result.length == 0 ){
                    ins.iocEvent('', mbusmma, 'error', 'in', {"Device":mbusmma,"action":"Heartbeat","result": "Fail"});
                    dcState = '';
                    var tm = 1000 + Math.floor((Math.random() * 10) + 1) * 200;
                    waitMiniSec(tm);
                    if ( dbg >= 0 ) console.log('%s motechat:PollDC: DC restart', ins.CurrentTime());
                    CallDcenterReset(function(result){
                        //if ( dbg >= 0 ) console.log('motechat:PollDC DCReset result=%s', JSON.stringify(result));
                        if ( result.ErrCode == mcerr.MC_OKCODE ){
                            // DC response OK
                    ...
                    if ( reply.result.length != edgetable.length && procDCPoll == true){
                        let ei = [];
                        edgetable.forEach(function(item){
                            let {SToken, EiName} = item;
                            ei.push({"SToken":SToken, "EiName":EiName});
                        });
                        console.log('motechat:PollDC table dc=%s,ei=%s', JSON.stringify(reply.result), JSON.stringify(ei));
                    }
                }
            }
            else {
...
var IssueMcState = function(state){
    if ( typeof state == 'object' ) console.log('mcState: %s', JSON.stringify(state));
    else console.log('McState: %s', state);
    if ( state ){
        let msg = 'mc: ' + state;
        if ( typeof wstatecb == 'function' ) wstatecb(msg);
        if ( typeof statecb == 'function' ) statecb(msg);
    }
}

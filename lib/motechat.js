 ...
 * @example
    var conf = { "AppName":"", "IOC":"", "DCenter":"", "UCenter":"", "AppKey":"", "UseWeb":"", "MotebusGW":"", "Keepalive":false }
	conf.AppName = 'myfunc';
 ...
 * @param {String} conf.Keepalive   Watch session with DC
...
//      Keepalive: watch the session with DC
...
exports.Open = function(conf, reg, callback){
    ...
    if ( conf.Keepalive ) {
        let keepalive = conf.Keepalive;
        if ( keepalive == true ) isPollDC = true;
        else if ( keepalive == false ) isPollDC = false;
    }
    console.log('motechat: keepalive=%s', isPollDC);
        ...
        CallDcenterReset(function(reply){
            ...
            if ( reply.ErrCode == mcerr.MC_OKCODE ){
                ...
                if ( regflag == true && reginfo ){
                    var data = {"SToken":reginfo.SToken,"EiToken":reginfo.EiToken,"WIP":mcwanip,"LIP":mclanip,"Web":webtype};
                    console.log('motechat:connDC reg data=%s', JSON.stringify(data));
                    //tryreg( data, opencb );
                    regfunc.start(data, 'user', opencb);
                }
                else {
                ...
                ins.GetmbWIP(function(result){
                    ...
                    if ( result.ErrCode == mcerr.MC_OKCODE ){
                        ins.On('message', MoteChatGetHandler);
                        if ( regflag == true && reginfo ){
                            let mcwanip = result.WANIP ? result.WANIP : dcwip;
                    ...
                    function(result){
                        ...
                        if ( Array.isArray(body) ) {
                            ...
                            if ( result.IN ) {
                                ...
                                // motebus reply
                                let rdata = {"IN":{"From":xdata.in.fm,"To":xdata.to,"State":result}};
                                cdata = [rdata];
                                rtype = 'err';
                            }
                        }
                        ...
                        if ( rtype == 'err' ){
                            let err = result.ErrMsg;
                            console.log('motechat:Send err=%s', JSON.stringify(err));
                        }
                        else {
                            if ( isPollDC == true ) StartWDtimer();
                        }
...
exports.Call = function(xrpc, cb){
    ...
    try {
        ...
        var info = GetEdgeInfo('stoken', stoken);
        if ( dbg >= 0 ) console.log('motechat:Call addr=%s,info=%s', JSON.stringify(addr),JSON.stringify(info));
        if ( info != null ){
                    ...
                    ins.CallXrpc( dcenter, 'call', dcData, timeout, waitreply, function(result){
                        ...
                        if ( Array.isArray(result) ) {
                            ...
                            if ( result.IN ) {
                                ...
                                // motebus reply
                                let rdata = {"IN":{"From":dcData.in.fm,"To":dcData.to,"State":result}};
                                cdata = [rdata];
                                rtype = 'err';
                            }
                        }
                        ...
                        if ( rtype == 'err' ){
                            let err = result.ErrMsg;
                            console.log('motechat:Call err=%s', JSON.stringify(err));
                        }
                        else {
                            if ( isPollDC == true ) StartWDtimer();
                        }
...
var DDNParser = function(ddn, topic){
    var ret;
    if ( ddn.indexOf(target_prefix) >= 0 ){

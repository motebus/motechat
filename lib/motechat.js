...
// Date: 2019/12/21
// Version: 1.8.2
// Update: change to async function
//         add mbSetConfig, mbGetConfig 

var exports = module.exports = {};
var ver = '1.8.2';
var update = '2019/12/21'
var appname = '';
var iocmma = '';
var webtype = '';
var dcenter = '';
...
var rcvcb = [];         // callback table of rcve when xmsg is comming 
var dbg = 0;
var isOpened = false;   // motebus open?
var openFlag = false;   // open function in process?
var regFlag = false;
var regfunc = new regCenter();
var rcveSS = new rcveCenter();
...
exports.Open = function(conf, reg, callback){
    ...
    if ( typeof cb == 'function' ){
        mchatStart( conf, reginfo, cb );
    }
    else {
        return new Promise((resolve) => {
            mchatStart( conf, reginfo, function(result){
                resolve(result);
            } );
        })
    }
...
var mchatStart = function(conf, reginfo, cb){
    if ( openFlag == false ){
        if ( isOpened == false ){
            openFlag = true;
            mcState = '';
            //console.log('motechat:open mchatStart');
            try {
                if ( conf ){
                    console.log('motechat:open conf=%s', JSON.stringify(conf));
                    mbusOpen(conf, function(result){
            ...
            if ( needset ) {
                if ( result.ErrCode == mcerr.MC_OKCODE ){
                    if ( result.result ){
                        ...
                        if ( CompareEdgeInfo( result.result, reginfo.EdgeInfo ) == true ) needset = false;
                    }
                    if ( needset ){
...
var mchatClose = function(cb){
    if ( mcState == 'open' ){
        ...
        if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.IN_Mbus_NotOpen,"ErrMsg":mcerr.IN_Mbus_NotOpen_Msg});
    }
}
...
exports.Publish = function(app, func, cb){
    if ( typeof cb == 'function' ){
        mchatPub(app, func, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatPub(app, func, function(result){
                resolve(result);
            });
        })
    }
...
var mchatPub = function(app, func, cb){
    if ( app && func ){
...
function regCenter()  {
    ...
    var _reg = function(data, msg, cb){
        ...
        if ( dcenter != '' ){
            ...
            let dcData = {"AppKey":appkey,"EiToken":data.EiToken,"SToken":data.SToken,"EiUMMA":mbusmma,"EiUPort":mbusport,"WIP":wanip,"LIP":lanip,"EdgeInfo":setei};
...
var mchatUnReg = function(data, cb){
    try {
        console.log('motechat: unreg data=%s', JSON.stringify(data));
        if ( data ) {
            let {SToken} = data;
...
var mchatSend = function(xmsg, cb){
    try {
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb([{"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg}]);
            return;    
...
var mchatCall = function(xrpc, cb){
    try {
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb([{"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg}]);
            return;    
        ...
        if ( selfinfo ) {
            ...
            if ( !Addr || !Data ){
                ...
                if ( Addr.mode == 'xs'){
                    ...
                    //console.log('motechat:Call uc addr=%s, xrpc=%s', JSON.stringify(Addr), JSON.stringify(xrpc));
                    UcFunc(selfinfo, xrpc, cb);
                }
...
var DDNParser = function(ddn, topic){
    ...
    else if ( ddn.indexOf(uc_prefix) >= 0 ){
        ret = TopicParser(topic);
    }
...
var XShareFunc = function(xsdata, cb){
    if ( dbg >= 1 ) console.log('motechat:XShareFunc data=', xsdata);
    let {Topic, Func, Data} = xsdata;
    if ( Topic && Func && Data ){
        if ( typeof cb == 'function' ){
            try {
                ins.XShareFunc( xsdata, (result) => {
                    if ( dbg >= 1 ) console.log('motechat:XShareFunc result=', result);
                    cb(result);
                })
            }
            catch(e){
                cb({"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":e.message});
            }
        }
        else {
            try {
                ...
                resolve({"ErrCode":mcerr.mcerr.MC_ERRCODE,"ErrMsg":e.message});
            }
        }
    }
    else {
...
var UcFunc = function(reg, udata, cb){
    if ( dbg >= 1 ) console.log('motechat:UcFunc data=%s', JSON.stringify(udata));
    try {
        let {EiMMA, SToken} = reg;
        let {Topic, Data} = udata;
        if ( EiMMA && Topic && Data ){
            let Func = Topic.substr(uc_prefix.length+1);
            if ( Func ) Func = Func.toLowerCase();
            //console.log('motechat:ucFunc Func=%s', Func);
            let Args = [];
            switch( Func ){
                case 'checkuser':
                    Func = 'ucCheckUser';
                ...
                case 'signup':
                    Func = 'ucSignup';
                    Args = [Data.UserName, Data.Password, Data.UserInfo];
                    break;
                case 'login':
                    Func = 'ucLogin';
                    Args = [Data.UserName, Data.Password, Data.KeepLogin];
                    break;
                case 'logout':
                    Func = 'ucLogout';
                    Args = [];
                ...
                case 'setusersetting':
                    ...
                    Args = [Data.KeyName, Data.Setting];
                    break;
                default:
                    Func = '';
                    break;    
            ...
            if (Func && Args){
                if ( ucState == 'start' ){
                    ...
                    console.log('motechat:UcFunc ucdata=%s', JSON.stringify(ucdata));
                    uc.UcCall( ucdata, function(reply){
                        let ret = GetUcResult(Func, reply);
                        console.log('UcCall ret=%s', JSON.stringify(ret));
                        if ( typeof cb == 'function' ) cb(ret);
                    ...
                    if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":"mc: uc modules not open"});
                }
            }
            else {
                if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg});
...
exports.GetAppSetting = function(data, cb){
    if ( typeof cb == 'function' ){
        mchatGetAppSetting(data, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatGetAppSetting(data, (result) => {
                resolve(result);
            });
        });
    }
...
var mchatGetAppSetting = function(data, cb){
    // data : {SToken}
    try {
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg});
...
exports.SetAppSetting = function(data, cb){
    if ( typeof cb == 'function' ){
        mchatSetAppSetting(data, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatSetAppSetting(data, (result) => {
                resolve(result);
            });
        });
    }
...
var mchatSetAppSetting = function(data, cb){
    try {
        // data: {SToken, Setting}
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg});
...
exports.GetQPin = function(data, cb){
    if ( typeof cb == 'function' ){
        mchatGetQPin(data, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatGetQPin(data, (result) => {
                resolve(result);
            });
        });
    }
...
var mchatGetQPin = function(data, cb){
    // data: {SToken}
    try {
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg});
...
exports.FindQPin = function(data, cb){
    if ( typeof cb == 'function' ){
        mchatFindQPin(data, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatFindQPin(data, (result) => {
                resolve(result);
            });
        });
    }
...
var mchatFindQPin = function(data, cb){
    // data: {SToken, Qpin}
    try {
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg});
...
var mchatSearch = function(data, cb){
    try {
    // data {SToken, Keyword}
        if ( dcState != 'open' ){
            if ( typeof cb == 'function' ) cb({"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg});
...
exports.SendLog = function(data, cb){
    if ( typeof cb == 'function' ){
        mchatSendLog(data, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatSendLog(data, (result) => {
                resolve(result);
            });
        });
    }
...
var mchatSendLog = function(data, cb){
    try {
...
exports.ListLogs = function(data, cb){
    if ( typeof cb == 'function' ){
        mchatListLogs(data, cb);
    }
    else {
        return new Promise((resolve) => {
            mchatListLogs(data, (result) => {
                resolve(result);
            });
        });
    }
...
var mchatListLogs = function(data, cb){
    try {
...
exports.mbCall = function(xrpc, cb){
    if ( typeof cb == 'function' ){
        ...
        return new Promise((resolve) => {
            mchatmbCall(xrpc, function(reply){
                resolve(reply);
            });
        });
    }
...
var mchatmbCall = function(xrpc, cb){
    try {
...
exports.mbSend = function(xmsg, cb){
    if ( typeof cb == 'function' ){
        ...
        return new Promise((resolve) => {
            mchatmbSend(xmsg, function(reply){
                resolve(reply);
            });
        });
    }
...
var mchatmbSend = function(xmsg, cb){
    try {
...
exports.Rcve = function(ddn, cb){
    let ret = rcveSS.addRcve(ddn, cb);
    return ret;
}

function rcveCenter() {
    this.addRcve = function(ddn, cb) {
        if ( ddn && cb ){
            if ( typeof cb == 'function' ) {
                let info = GetEdgeInfo('ddn', ddn, 'reg');
                if ( info ) {
                    let index = _chkrcvq(ddn);
                    if ( index >= 0 ) {
                        rcvcb[index].cb = cb;
                    }
                    else {
                        let rcv = {"DDN":ddn,"cb":cb};
                        rcvcb.push(rcv);
                    }
                    return {"ErrCode":mcerr.MC_OKCODE,"ErrMsg":mcerr.MC_OKMSG};
                }
                else {
                    return {"ErrCode":mcerr.MC_NotOpen,"ErrMsg":mcerr.MC_NotOpen_Msg};
                }
            }
            else {
                return {"ErrCode":mcerr.MC_NoRcveFunc,"ErrMsg":mcerr.MC_NoRcveFunc_Msg};
            }
        }
        return {"ErrCode":mcerr.MC_InvalidData,"ErrMsg":mcerr.MC_InvalidData_Msg};
    }
    this.RmRcve = function(ddn){
        if ( ddn ){
            let index = _chkrcvq(ddn);
            if ( index >= 0 ) {
                rcvcb.splice(index, 1);
            }
            return {"ErrCode":mcerr.MC_OKCODE,"ErrMsg":mcerr.MC_OKMSG};
        }
        else {
    ...
    this.ChkRcve = function(ddn){
        let index = _chkrcvq(ddn);
        return index;
    }
    var _chkrcvq = function( ddn ){
        let len = rcvcb.length;
        if ( len > 0 ){
            for ( let i = 0; i < len; i++ ){
                let {DDN} = rcvcb[i];
                if ( DDN == ddn ) return i;
            ...
            if ( ddnlist.length > 0 ){
                ...
                for ( var i = 0; i < ddnlist.length; i++ ){
                    ...
                    if ( ddnlist[i].Web == ''){
                        ...
                        if ( !finish ){
                            let inctl = {"From":from,"To":to,"msgtype":msgtype};
                            let index = rcveSS.ChkRcve(toddn);
                            if ( index >= 0 ) {
                                let callback = rcvcb[index].cb;
                                if ( typeof callback == 'function' ) {
                                    callback(ch, inctl, data, function(reply){
                                        if ( dbg >= 1 ) console.log('motechat:MoteChatGetHandler rcve reply=%s',JSON.stringify(reply));
                                        if ( reply.response ) InTraceResp(reply);
                                        if ( typeof cb == 'function' ) cb(reply);    
                                    });
                                }
                                else {
                                    let ret = {"ErrCode":mcerr.MC_NoRcveFunc,"ErrMsg":mcerr.MC_NoRcveFunc_Msg,"By":mbusmma};
                                    cb(ret);
                                }
                            }
                            else {
                ...
                let ret = {"ErrCode":mcerr.MC_NoMatchDDN,"ErrMsg":mcerr.MC_NoMatchDDN_Msg,"By":mbusmma};
                console.log('motechat:MoteChatGetHandler ret=%s',JSON.stringify(ret));
                cb(ret);
            }
        }
        catch(err){
            let ret = {"ErrCode":mcerr.MC_ERRCODE,"ErrMsg":err.message,"By":mbusmma};
            console.log('motechat:MoteChatGetHandler ret=%s',JSON.stringify(ret));
            cb(ret);
        }
    }
...
exports.mbGetConfig = function(xconf){

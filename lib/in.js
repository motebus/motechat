...
exports.SetCluster = function(name, list, type){
	setcluster(name, list, type)
}
		...
		if (result) {
			...
			xmsg.on('message', function(msg) {
				...
				if ( msg.body.eventType ){
					...
					ret = {"ErrCode":inerr.UnknownData,"ErrMsg":inerr.UnknownData_Msg};
					replyxmsg(msg.head, ret, DefaultXmsgTimeout);
				}
			});
		}
		else {
...
var mbusInfo = function( cb, errcb ){
	...
	.catch(function(err){
		...
		if ( typeof errcb == 'function') errcb({"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"in: " + err.message});
	});	
}
...
var sendxmsg = function( mma, body, files, t1, t2, cb ){
	...
	if ( mbstate == 'opened' ){
		...
		function(err, tkinfo) { 
			if (err) {
				...
				if (state != 'Reply') {
					...
					if ( t2 == 0 && state == 'Read') {
						if ( dbg >= 0 ) console.log("--%s: sendxmsg: to=%s,state=%s", CurrentTime(), mma, state);
		...
		console.log("--%s: sendxmsg error: %s", CurrentTime(), JSON.stringify(errmsg))
	}
}
...
var replyxmsg = function(head, body, t1, cb ){
	...
	if ( dbg >= 1 ) console.log('--%s: replyxmsg body=%s', CurrentTime(), JSON.stringify(body));
	if ( mbstate == 'opened'){
		xmsg.reply( head, body, [], 10/*Prio*/, t1, 0, 
			function(err, tkinfo) {
				if (err) {
					console.log("--%s: replyxmsg: err=%s", CurrentTime(), err.message);
					if ( typeof cb == 'function' ) cb({"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"in: " + err.message});
				} else {
					state = tkinfo.state;
					if ( dbg >= 1 ) console.log("--%s: replyxmsg: state= %s", CurrentTime(), state);
		...
		if ( typeof cb == 'function'){
			var errmsg = {"ErrCode":inerr.IN_Mbus_NotOpen,"ErrMsg":inerr.IN_Mbus_NotOpen_Msg};
			cb(errmsg);
			console.log("--%s: replyxmsg: err= %s", CurrentTime(), JSON.stringify(errmsg));
		}
	}
}
...
var setcluster = function(name, serverlist, stype){

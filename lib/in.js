...
exports.GetmbWIP = function(cb){
	if ( typeof cb == 'function') {
		mbusInfo( function(reply){
			if ( dbg >= 1 ) console.log('in:GetmbWIP mbusinfo reply=%s', JSON.stringify(reply));
...
exports.XShareFunc = async function(cmd, cb){
	try {
		if ( dbg >= 1 ) console.log('in:XShareFunc cmd=%s', JSON.stringify(cmd));
		let {Topic, Func, Data} = cmd;
		if ( Topic ) Topic = Topic.replace('xs://', '');
		switch(Topic) {
			case 'config':
				result = await xConfigFunc(Func, Data);
				if ( dbg >= 1 ) console.log('in:xShareFunc xConfig result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			case 'cached':
				result = await xCachedFunc(Func, Data);
				if ( dbg >= 1 ) console.log('in:xShareFunc xCached result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			case 'bucket':
				result = await xBucketFunc(Func, Data);
				if ( dbg >= 1 ) console.log('in:xShareFunc xBucket result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			case 'secret':
				result = await xSecretFunc(Func, Data);
				if ( dbg >= 1 ) console.log('in:xSecretFunc xSecurt result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			default:
				if ( typeof cb == 'function' ) cb({"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg,"info":cmd});
				break;
		}
	}
	catch(e){
		console.log('in:xSecretFunc error=%s', e.message);
		if ( typeof cb == 'function' ) cb({"ErrCode":inerr.IN_ERRCODE,"ErrMsg":e.message});
	}
}

const xConfigFunc = async function(func, data){
	if ( dbg >= 1 ) console.log('in:xConfigFunc func=%s,data=%s', func, JSON.stringify(data));
	let payload = null;
	...
	if ( func == 'get' ) {
		...
		if ( catalog && idname ){
			let reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG,"result":null};
			let ret = await xshare.getConfig(catalog, idname);
			if ( ret ) reply.result = ret;
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg,"info":func};
		}
	}
	else if ( func == 'set'){
		...
		if ( catalog && idname && data ) {
			let reply = null;
			let ret = await xshare.setConfig(catalog, idname, data);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"set error"};
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg,"info":func};
		...
		return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":"func error"};
	}
}

const xCachedFunc = async function(func, xdata){
	if ( dbg >= 1 ) console.log('in:xCachedFunc func=%s,xdata=%s', func, JSON.stringify(xdata));
	let payload = null;
	...
	if ( func == 'get' ) {
		...
		if ( catalog && idname ){
			let reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG,"result":null};
			let ret = await xshare.getCached(catalog, idname);
			if ( ret ) reply.result = ret;
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg};
		}
	}
	else if ( func == 'set'){
		let {catalog, idname, data} = payload;
		if ( catalog && idname && data ){
			let reply = null;
			let ret = await xshare.setCached(catalog, idname, data);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"set error"};
			return reply;
		}
		...
		if ( catalog && idname ){
			let reply = null;
			let ret = await xshare.removeCached(catalog, idname);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"remove error"};
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg};
		...
		if ( catalog ){
			let reply = null;
			let ret = await xshare.clearCached(catalog);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"clear error"};
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg};
...
const xBucketFunc = async function(func, xdata){
	if ( dbg >= 1 ) console.log('in:xBucketFunc func=%s,xdata=%s', func, JSON.stringify(xdata));
	let payload = null;
	if ( typeof xdata == 'string' ) payload = JSON.parse(xdata);
	else payload = xdata;
	if ( func == 'get' ) {
		...
		if ( catalog && idname ){
			let reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG,"result":null};
			let ret = await xshare.getBucket(catalog, idname);
			if ( ret ) reply.result = ret.toString(datatype);
			if ( dbg >= 1 ) console.log('in:xBuketFunc getBucket result=%s', reply.result);
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg};
		...
		if ( catalog && idname && data ){
			...
			//ret = await xshare.setBucket(catalog, idname, buf);
			
			if ( Buffer.isBuffer(data) )
				ret = await xshare.setBucket(catalog, idname, data);
			else {
				var buf;
		...
		if ( catalog ){
			let reply = null;
			let ret = await xshare.listBucket(catalog);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG,"result":ret};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"list error"};
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg};
		...
		if ( catalog && idname ){
			let reply = null;
			let ret = await xshare.removeBucket(catalog, idname);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"remove error"};
			return reply;
		}
...
const xSecretFunc = async function(func, xdata){
	if ( dbg >= 1 ) console.log('in:xSecretFunc func=%s,xdata=%s', func, JSON.stringify(xdata));
	let payload = null;
	if ( typeof xdata == 'string' ) payload = JSON.parse(xdata);
	else payload = xdata;
	if ( func == 'get' ) {
		...
		if ( catalog && idname && password ){
			let reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG,"result":null};
			let ret = await xshare.getSecret(catalog, idname, password);
			if ( ret ) reply.result = ret;
			return reply;
		}
		else {
		...
		if ( catalog && idname && data && password ) {
			let reply = null;
			let ret = await xshare.setSecret(catalog, idname, data, password);
			if ( ret ) reply = {"ErrCode":inerr.IN_OKCODE,"ErrMsg":inerr.IN_OKMSG};
			else reply = {"ErrCode":inerr.IN_ERRCODE,"ErrMsg":"set error"};
			return reply;
		}

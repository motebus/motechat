	...
	motebus.on('ready', function() {
		...
		openxmsg( app, function(result){
            ...
            if ( result.ErrCode == inerr.IN_OKCODE ){
				...
				var tm = Math.floor((Math.random() * 10) + 1) * 100;
				setTimeout(function(){
	...
	xmsg.open( userid, 'zaq1234', false, function( err, result ){
		...
		if (result) {
			...
			xmsg.on('message', function(msg) {
				...
				if ( dbg >= 1 ) console.log('--%s: message from=%s', CurrentTime(), JSON.stringify(msg.head));
				if ( dbg >= 1 ) console.log('--%s: message body=%s', CurrentTime(), JSON.stringify(msg.body));
				if ( msg.body.in.msgtype && msg.body.in.msgtype == 'in' ){
					incmdparser( msg );
				}
...
var sendxmsg = function( mma, body, files, t1, t2, cb ){
	...
	if ( dbg >= 1 ) console.log('--%s: sendxmsg body=%s', CurrentTime(), JSON.stringify(body));
	if ( mbstate == 'opened' ){
...
var iocEvent = function( evId, evSource, evType, evClass, evBody ){
	...
	try {
		...
		if ( dbg >= 1 ) console.log('in:iocEvent evdata=%s', JSON.stringify(evData));

...
exports.Open = function( conf, cb ){
    try {
		...
		if ( conf ){
			...
			iocmma = conf.IOC ? conf.IOC : 'ioc@ioc.ypcloud.com';
			if ( iocmma ){
		...
		function(err, tkinfo) { 
			if (err) {
				...
				if (state != 'Reply') {
					...
					if ( dbg >= 0 ) console.log("--%s: sendxmsg Reply from: %s", CurrentTime(), JSON.stringify(tkinfo.msg.head) );

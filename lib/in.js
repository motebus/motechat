...
// Date: 2019/09/27
// Version: 1.08
// Update IOC queue, remove mlog

var exports = module.exports = {};
var ver = '1.08';
var update = '2019/09/27';
var appname = '';
var iocmma = '';
var mbusgw = '';
var mbusport;
...
exports.Open = function( conf, cb ){
    try {
		...
		if ( conf ){
			...
			mbusgw = conf.MotebusGW ? conf.MotebusGW : '127.0.0.1';
			if ( mbusgw.indexOf(':') > 0 ){
				...
				if ( dbg == 0 ) {
					if ( result && Array.isArray(result) ){
						for ( let i = 0; i < result.length; i++ ){
							if ( result[i] && result[i].IN )
								console.log("--%s: CallXrpc to:%s, state:%s", CurrentTime(), JSON.stringify(result[i].IN.To), JSON.stringify(result[i].IN.State) );
						}
					}
				}
				else if ( dbg >= 1 ) {
...
var mbusOpen = function( app, cb ){
	...
	motebus.on('ready', function() {
		...
		var dm = Math.floor((Math.random() * 10) + 1) * 500;
...
var SaveMyInfo = function( info ){
	try {
		...
		console('SaveMyInfo error: %s', e.message);
		return false;
	}
}
					...
					if ( dbg == 0 ) {
						...
						if ( body && Array.isArray(body) ){
							for ( let i = 0; i < body.length; i++ ){
								if ( body[i] && body[i].IN )
									console.log("--%s: sendxmsg to:%s, state:%s", CurrentTime(), JSON.stringify(body[i].IN.To), JSON.stringify(body[i].IN.State) );
							}
						}
					}
					else {
	...
	if ( iocsdq.length > 0 && (mbstate == 'opened'|| mbstate == 'opened2' )){
		...
		iocEvent(evId, evSource, evType, evClass, evBody, function(result){
			...
			if ( result == 'Read' ){
				...
				let tm = 3000 + Math.floor((Math.random() * 10) + 1) * 100;;
				setTimeout(function(){
					iocEventHandler();
				}, tm);
			}

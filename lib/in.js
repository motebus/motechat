...
exports.XShareFunc = async function(cmd, cb){
	try {
		console.log('in:XShareFunc cmd=%s', JSON.stringify(cmd));
		let {Topic, Func, Data} = cmd;
		if ( Topic ) Topic = Topic.replace('xs://', '');
		switch(Topic) {
			case 'config':
				result = await xConfigFunc(Func, Data);
				console.log('in:xShareFunc xConfig result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			case 'cached':
				result = await xCachedFunc(Func, Data);
				console.log('in:xShareFunc xCached result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			case 'bucket':
				result = await xBucketFunc(Func, Data);
				console.log('in:xShareFunc xBucket result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
			case 'secret':
				result = await xSecretFunc(Func, Data);
				console.log('in:xSecretFunc xSecurt result=', result);
				if ( typeof cb == 'function' ) cb(result);
				break;
		}
	}
...
const xConfigFunc = async function(func, data){
	console.log('in:xConfigFunc func=%s,data=%s', func, JSON.stringify(data));
	let payload = null;
...
const xCachedFunc = async function(func, xdata){
	console.log('in:xCachedFunc func=%s,xdata=%s', func, JSON.stringify(xdata));
	let payload = null;
...
const xBucketFunc = async function(func, xdata){
	console.log('in:xBucketFunc func=%s,xdata=%s', func, JSON.stringify(xdata));
	let payload = null;
	if ( typeof xdata == 'string' ) payload = JSON.parse(xdata);
	else payload = xdata;
	if ( func == 'get' ) {
		...
		if ( catalog && idname ){
			...
			console.log('in:xBuketFunc getBucket result=%s', reply.result);
			return reply;
		}
		else {
			return {"ErrCode":inerr.IN_InvalidData,"ErrMsg":inerr.IN_InvalidData_Msg};
...
const xSecretFunc = async function(func, xdata){
	console.log('in:xSecretFunc func=%s,xdata=%s', func, JSON.stringify(xdata));
	let payload = null;
	if ( typeof xdata == 'string' ) payload = JSON.parse(xdata);
	else payload = xdata;
	if ( func == 'get' ) {

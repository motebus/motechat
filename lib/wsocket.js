                ...
                if ( func == 'send' ){
                  try {
                    if ( typeof msg.body.target != 'undefined' && typeof msg.body.data != 'undefined' ){
                      ...
                      var xmsgctl = {"SToken":stoken,"From":ddn,"Target":target,"Data":data,"SendTimeout":null,"WaitReply":null};
  ...
  if ( msgtype == '' ){
    //if ( ch == 'xrpc'){
      ...
      if ( ix >= 0 ){
        ...
        console.log('wsocket:InmsRcve to=%s',to);
        if ( typeof cb == 'function' ) cb({"ErrCode":wserr.WS_NoMatchDDN,"ErrMsg":wserr.WS_NoMatchDDN_Msg,"DDN":to});
      }
    //}
  }
  else {
    // not normal data
    if ( typeof cb == 'function' ) cb({"ErrCode":wserr.WS_InvalidData,"ErrMsg":wserr.WS_InvalidData_Msg});  
  }
}

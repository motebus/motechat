...
exports.Init = function( mc, webserver, cb, appcb, msgcb ){
  ...
  io = require('socket.io')(webserver,
    {pingTimeout:120000,pingInterval:30000,cookie:true,transports:['websocket','polling']}
  );
  ...
  io.on('connection', function (socket) {
    if ( dbg >= 1 ) console.log('->%s: %s: socket conn', CurrentTime(), socket.id );
      ...
      console.log('socket conn: socketid=%s,ip=%s', socket.id,rmtip);
              ...
              if ( func == 'send' ){
                ...
                try {
                  ...
                  mochat.Reg(reginfo, function(reply){
                    ...
                    else console.log('wsocket:reg reply=%s', reply.ErrMsg);
                    if ( reply.ErrCode == wserr.WS_OKCODE ){
                      ...
                      if ( reply.result ){
                        ...
                        if ( ddn && stoken ){
                          ...
                          if ( typeof ack == 'function' ){
                            let err = {"ErrCode":wserr.WS_ERRCODE,"ErrMsg":"ws: Reg error","info":reginfo};
...
var InState = function(state, ddn){
  if ( ddn ){
    ...
    var client, msg = '';
    if ( state == 'opened2' ) msg = 'mbus re-opened';
    if ( state == 'off' ) msg = 'mbus off';
    if ( state == 're-reg' ) msg = 'dc re-registered'
    if ( msg ){

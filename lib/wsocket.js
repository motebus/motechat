      ...
      if ( dbg >= 0 ) console.log('->%s: %s: socket conn', CurrentTime(), socket.id );
        ...
        socket.on('request', function(msg, ack) {
            ...
            if ( typeof msg.func == 'string' ){
                ...
                if ( func == 'send' ){
                  try {
                    if ( msg.body.topic && msg.body.data ){
                      var topic = msg.body.topic;
                      var data = msg.body.data;
                      var stoken = GetSocketAttr('stoken', socket.id);
                      var xmsgctl = {"SToken":stoken,"Topic":topic,"Data":data,"SendTimeout":null,"WaitReply":WS_WAITREPLY_TIMEOUT};
                      mochat.Send(xmsgctl, function(reply){
                        if ( dbg >= 0 ) console.log('wsocket:send reply=%s', JSON.stringify(reply));
                  ...
                  try {
                    if ( msg.data.topic && msg.data.func && msg.data.args ){
                      var topic = msg.data.topic;
                      var cfunc = msg.data.func;
                      var data = msg.data.args;
                      var stoken = GetSocketAttr('stoken', socket.id);
                      var xrpcblk = {"SToken":stoken,"Topic":topic,"Func":cfunc,"Data":data,"SendTimeout":null,"WaitReply":WS_WAITREPLY_TIMEOUT};
                      mochat.Call(xrpcblk, function(reply){
                        if ( dbg >= 1 ) console.log('wsocket:call reply=%s', JSON.stringify(reply));
                        if ( typeof ack == 'function') ack(reply);
                      });
                    }
                    else {
  ...
  if ( inctl ){
    ...
    if ( ix >= 0 ){
      ...
      client.emit( 'message', {"method":ch,"ctl":inctl,"data":data}, function(reply){
        console.log('wsocket:InmsgRcve: reply=%s', JSON.stringify(reply));
        cb(reply);
      });
    }
    else {
      console.log('wsocket:InmsRcve ddn=%s',ddn);
      cb({"ErrCode":wserr.WS_NoMatchDDN,"ErrMsg":wserr.WS_NoMatchDDN_Msg,"DDN":ddn});
    }
  }
  else {
    cb({"ErrCode":wserr.WS_InvalidData,"ErrMsg":wserr.WS_InvalidData_Msg,"DDN":to});
  }
}
...
var GetSocketAttr = function(attr, sid){
  ...
  else if ( dbg >= 1 ) console.log('wsocket:SetSocketAttr error');
  return ret;  
}

...
exports.Init = function( mc, webserver, wsport, webpath, cb, syscb ){
      ...
      if ( dbg >= 0 ) console.log('->%s: %s: socket conn', CurrentTime(), socket.id );
        ...
        socket.on('request', function(msg, ack) {
            ...
            if ( typeof msg.func == 'string' ){
                ...
                if ( func == 'send' ){
                  try {
                    if ( (msg.body.ddn || msg.body.topic) && msg.body.data ){
                      var ddn = msg.body.ddn;
                      var topic = msg.body.topic;
                      var data = msg.body.data;
                      var stoken = GetSocketAttr('stoken', socket.id);
                      var xmsgctl = {"SToken":stoken,"DDN":ddn,"Topic":topic,"Data":data,"SendTimeout":WS_SENDXMSG_TIMEOUT,"WaitReply":WS_WAITREPLY_TIMEOUT};
                  ...
                  try {
                    if ( (msg.data.ddn || msg.data.topic) && msg.data.func && msg.data.args ){
                      var ddn = msg.data.ddn;
                      var topic = msg.data.topic;
                      var cfunc = msg.data.func;
                      var data = msg.data.args;
                      var stoken = GetSocketAttr('stoken', socket.id);
                      var xrpcblk = {"SToken":stoken,"DDN":ddn,"Topic":topic,"Func":cfunc,"Data":data,"SendTimeout":WS_CALLXRPC_TIMEOUT,"WaitReply":WS_WAITREPLY_TIMEOUT};

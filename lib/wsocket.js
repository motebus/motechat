        ...
        socket.on('request', function(msg, ack) {
            ...
            if ( typeof msg.func == 'string' ){
                ...
                if ( func == 'regdc'){
                  ...
                  mochat.GetDeviceInfo(msg.data, function(reply){
                  ...
                  mochat.SetDeviceInfo(msg.data, function(reply){
                  ...
                  try {
                    ...
                    mochat.CallXrpc(xrpcblk, function(reply){
                      //console.log('wsocket:call reply=%s', JSON.stringify(reply));
                      if ( typeof ack == 'function') ack(reply);
                    });
                    /*
                    if ( typeof xargs.target != 'undefined' && typeof xargs.data != 'undefined' ){
                      var ddn = GetSocketAttr('ddn', socket.id);
                      var stoken = GetSocketAttr('stoken', socket.id);
                      var xdata = {"in":{"fm":{"DDN":ddn},"msgtype":""},"target":xargs.target,"data":xargs.data};
                      var xrpcblk = {"SToken":stoken,"App":msg.data.app,"Func":msg.data.func,"Data":xdata};
                      mochat.CallXrpc(xrpcblk, function(reply){
                        //console.log('wsocket:call reply=%s', JSON.stringify(reply));
                        if ( typeof ack == 'function') ack(reply);
                      });
                      ...
                      var stoken = GetSocketAttr('stoken', socket.id);
                      var xrpcblk = {"SToken":stoken,"App":msg.data.app,"Func":msg.data.func,"Data":msg.data.args};
                      mochat.CallXrpc(xrpcblk, function(reply){
                        //console.log('wsocket:call reply=%s', JSON.stringify(reply));
                        if ( typeof ack == 'function') ack(reply);
                      });

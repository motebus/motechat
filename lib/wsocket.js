            ...
            if ( func == 'send' ){
              try {
                if ( (msg.body.ddn || msg.body.topic) && msg.body.data ){ // v2.0
                  let {ddn, topic, data} = msg.body;
                  let stoken = GetSocketAttr('stoken', socket.id);
                  if ( stoken && (ddn || topic) && data ) {
              ...
              try {
                console.log('wsocket:call msg=', msg);
                if ( (msg.data.ddn || msg.data.topic) && msg.data.func && msg.data.args ){
                  let {ddn, topic, func, args} = msg.data;
                  let stoken = GetSocketAttr('stoken', socket.id);
                  if ( stoken && (ddn || topic) && func && args ){

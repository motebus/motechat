...
/*
 * Module: Start session with device center
 * data    the information for session, {EiToken,SToken}
 * data.EiToken device token
 * data.SToken  app token 
...
var NewEdgeInfo = function (stoken, appkey, eitoken, eiumma, eiuport, ddn) {
    var info = {
        "SToken": stoken, "AppKey": appkey, "EiToken": eitoken, "EiUMMA": eiumma, "EiUPort": eiuport,
...
var RemoveEdgeInfo = function (skey) {
    ...
    for (var i = 0; i < edgetable.length; i++) {
        ...
        if (skey == stoken) {
            edgetable.sGetEdgeInfoplice(i, 1);
            return true;
        }
    }
    return false;
